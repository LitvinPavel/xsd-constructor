...
export function generateXMLWithValidation(schema: XSDSchema): string {
  ...
  if (!rootElement || !rootElement.name || typeof rootElement.name !== 'string') {
...
function processElementWithValidation(parent: any, element: any): void {
  ...
  if (element.complexType?.complexContent && element.value) {
    processComplexTypeByDefinition(currentNode, element.type, element.value, element);
  ...
  if (elementDefinition?.complexType?.complexContent?.extension?.base === 'ReqElement') {
    ...
    for (const field of reqElementFields) {
      ...
      if (fieldValue !== undefined && fieldValue !== null && fieldValue !== '') {
        ...
        if (field === 'ReqElementData' && 
            (complexValue.attributes?.ReqElementType === 'графическое изображение' || 
